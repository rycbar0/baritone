...
public class CraftingProcess extends BaritoneProcessHelper implements ICraftingProcess {
    private int amount;
    ...
    public synchronized PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        if (calcFailed) {
            ...
            try {
                ...
                if (!canCraftInInventory(recipes.get(0)) && !(ctx.player().containerMenu instanceof CraftingMenu)) {
    ...
    public String displayName0() {
        return "Crafting " + amount + "x " + recipes.get(0).getResultItem(ctx.minecraft().getConnection().registryAccess()).getDisplayName();
    }

    @Override
    public void craft(List<CraftingRecipe> recipes, int amount) {
        this.recipes = recipes;
        this.amount = amount;
        clearToPush = true;
        logDirect("Crafting now " + amount + "x [" + recipes.get(0).getResultItem(ctx.minecraft().getConnection().registryAccess()).getDisplayName() + "]");
    }

    @Override
    public List<CraftingRecipe> getCraftingRecipes(Item item, boolean allCraftingRecipes) {
        List<CraftingRecipe> recipes = new ArrayList<>();
        for (CraftingRecipe recipe : BuiltInRegistries.RECIPE_TYPE.get(RecipeType<CraftingRecipe.class>)            CraftingManager.REGISTRY) {
